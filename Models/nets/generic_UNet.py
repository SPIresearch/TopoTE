...
class InitWeights_Xavier:
    """
    Xavier init for Conv/ConvTranspose (2D/3D).
    distribution: 'normal' or 'uniform'
    gain: usually 1.0, or nn.init.calculate_gain('relu') / for leakyrelu use calculate_gain('leaky_relu', neg_slope)
    """
    def __init__(self, gain=1.0, distribution="normal"):
    ...
    def __call__(self, m):
        import torch.nn as nn
        if isinstance(m, (nn.Conv3d, nn.Conv2d, nn.ConvTranspose3d, nn.ConvTranspose2d)):
            if self.distribution in ("uniform", "xavier_uniform"):
                nn.init.xavier_uniform_(m.weight, gain=self.gain)
            else:
                nn.init.xavier_normal_(m.weight, gain=self.gain)
            if m.bias is not None:
                nn.init.constant_(m.bias, 0.0)
